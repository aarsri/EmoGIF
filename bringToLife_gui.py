...
if __name__ == '__main__': # Aarohi Srivastava
    emotion = entry(text='Pick one: "excited", "gloomy", "happy", "nostalgic", "confused", "love", "whimsical", or "peaceful"', placeholder='type your choice here...', title='Step 1: Pick an emotion among:', width=330, height=120, timeout=None)
    im_fp = file_selection(multiple=False, directory=False, save=False, confirm_overwrite=False, filename=None, title='Step 2: Select your input selfie.', width=330, height=120, timeout=None) 
    output_fn = entry(text='Pick an output file name (ex: sally-happy.mp4)', placeholder='type here...', title='Step 3: Pick the output file name.', width=330, height=120, timeout=None)
    message(title='Please wait ~2 min for your video.', text="Great, we got it! You will find your video in livePortraits-master/video_generated/ after about 2 minutes!", width=330, height=120, timeout=5)
 
    if "excited" in emotion:
        emotion = 'excited'
    elif "gloomy" in emotion:
        emotion = "gloomy"
    elif "happy" in emotion:
        emotion = "happy"
    elif "nostalgic" in emotion:
        emotion = "nostalgic"
    elif "confused" in emotion:
        emotion = "confused"
    elif "love" in emotion:
        emotion = "love"
    elif "whimsical" in emotion:
        emotion = "whimsical"
    elif "peaceful" in emotion:
        emotion = "peaceful"
    startCap(im_fp, emotion, output_fn) #open video capture object and start
    message(title='Ready!', text="Your video is ready! Find it here: livePortraits-master/video_generated/" + output_fn, width=330, height=120, timeout=None)
    exit()
